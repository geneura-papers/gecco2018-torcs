...
The process of creating a car that wins races in a racing simulator
involves an optimization process that must use what is known about the
race to obtain good results in a priori unknown
environments. Evolutionary algorithms need a fitness function,
however, to carry out that optimization process, that is why in this
paper we set out to design and test several fitness functions that
would eventually make a car win in all possible scenarios, or at least
in as many as possible. What we are optimizing is a set of two fuzzy
controllers, which are computationally complete and have been proved
in the past to yield good results; however in this paper we will be
using some heuristics to first select the track in which solo
optimization is going to be run and then which combination of the
different measures taken during those races will eventually yield the
best results when in a real race.
